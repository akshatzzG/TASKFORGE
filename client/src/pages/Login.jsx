...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-base px-4 relative overflow-hidden">
      <div className="absolute top-1/4 left-1/3 w-96 h-96 bg-violet-glow rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-md w-full relative animate-fade-in-up">
        ...
        <div className="bg-surface border border-border-base rounded-2xl p-8">
          ...
          <p className="text-text-secondary text-sm mb-6">Sign in to your workspace</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-1.5 uppercase tracking-wide">Workspace</label>
              <input
        ...
        <p className="text-sm text-text-muted mt-6 text-center">
          New here? <Link to="/register" className="text-violet-400 font-medium hover:text-violet-300">Create a workspace</Link>
        </p>
      </div>
    </div>
  );
